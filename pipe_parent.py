...
if sys.version_info[0] > 2:
    if sys.platform == "win32":
        os.set_handle_inheritable(read_side_id, True)
        os.set_handle_inheritable(write_side_id, True)
    else:
...
child = Popen([python, "pipe_child.py", str(read_side_id), str(write_side_id)])
